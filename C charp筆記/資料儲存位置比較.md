...
Session.Abandon();//移除 session的所有變數，且會觸發 Session_End 事件，把 Session["yoyo"]整個砍了
...
HttpCookie yoCookie = newHttpCookie("test");//用new宣告一個型別為HttpCookie的yocookie  
...
關掉瀏覽器就消失(關掉分頁不會)。也可設定存活時間(預設為20分鐘)
